...
class Model<T: Object, M: Model<any, any>> {
    ...
    update(data: T): M {
        const newData = this._data.mergeDeep(data).merge({updatedAt: new Date()});

        const result = this.constructor.updateSchema.validate(newData.toJS());
